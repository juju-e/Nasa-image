...
import weasyprint         # convert to pdf
app=Flask(__name__)
...
def main():
 ...
 if form.validate_on_submit():     # if the user submits the form
  res=requests.get("https://api.nasa.gov/planetary/apod?api_key=<your_key>&date="+str(form.entry.data)).json()
  ...
  preview_file=open(str(date)+'.html','w')            # Saving the preview file as a html file
  preview_file.write(preview)                         #
  preview_file.close()                                #
  file=open(str(date)+'.pdf','wb').write(weasyprint.HTML(str(date)+'.html').write_pdf()) # converting to pdf
  return preview
